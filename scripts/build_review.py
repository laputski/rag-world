...
<header class="top">
  ...
  <p class="lede">Уровень зрелости вычисляется правилом и воспроизводится повторным запуском. Значение измерения так проверить нельзя: это вывод из текста статьи, и убедиться в нём можно только через рассуждение, которое к нему привело. Здесь оно записано целиком.</p>
  <p class="lede">У каждого пункта разделено <b>что делает система</b> и <b>почему из этого следует значение</b>: первое проверяется по источнику, второе — по схеме измерений. Отдельно названо значение, которое не подошло, — без него выбор выглядел бы единственно возможным.</p>
  <p class="lede">Пункты, где источник допускает другое прочтение, помечены. Решение по ним принято и записано; помечены они не как вопрос к вам, а как место, где я мог ошибиться правдоподобно.</p>
</header>
...
<div class="tally">
  ...
  <div class="cell open"><span class="num">{questioned}</span><span class="lab">спорных прочтений</span></div>
  <div class="filters">
...
<p class="foot">Свидетельства изложены моими словами со ссылкой на раздел источника, а не дословными выдержками. Значения на странице сверяются с реестром при сборке: обоснование, разошедшееся с данными, останавливает её.</p>
</div>

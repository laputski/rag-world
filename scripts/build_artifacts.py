...
def _latest_metric(points: list[store.MetricPoint], metric: str) -> float | None:
    ...
    У записи может быть несколько работ, и каждая измеряется отдельно, поэтому
    на свежайшую дату приходится не одна точка, а несколько. Берётся
    наибольшая, и выбор не произволен:

    * брать первую попавшуюся нельзя — величина зависела бы от порядка строк в
    ...
    * складывать нельзя — препринт и его конференционная версия существуют в
    ...
    Наибольшая — это внимание к самой заметной работе записи. Она не
    завышается: такое измерение действительно существует.
    """
    relevant = [p for p in points if p.metric == metric]
    if not relevant:
        return None
    freshest = max(p.measured_at for p in relevant)

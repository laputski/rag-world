...
/**
 * Схема здесь проверяется на внутреннюю связность, а её размер не закрепляется
 * числом.
 *
 * Закреплённое «26» уже назвало верное значение ошибкой: схема законно выросла
 * до двадцати восьми, а тест упал на сгенерированном файле, править который
 * нельзя. Совпадение с первоисточником сторожит
 * `tests/architecture/test_schema_module_in_sync.py`: у него есть доступ к
 * декларации на Python, и он сверяет модуль с ней целиком. Интерфейсу первого
 * источника не видно, поэтому здесь проверяется то, что видно ему.
 */
